...
<body>
 ...
 $countorg=mysql_query("select count(organizer_email) from organizer")or die('count query error');
   ...
   $query=mysql_query(" select user.fristname,user.lastname,user.email,organizer.companyname,organizer.country,organizer.city,organizer.logo from user join organizer where user.email=organizer.organizer_email LIMIT 20,41 ") or die('organizer    table selection query error');
...
if(mysql_num_rows($query)>0){
	   	...
	   	?>
							...
							<table class="table">
								<tbody>
									...
									<tr>
										...
										<td> <a href="deleteorganizer.php?email=<?php echo $rows[2];?>&&page=<?php echo 'organizer1.php';?>" onclick="return window.confirm('Are you sure to delete')"> <button type="button" class="btn btn-danger">Delete</button></a></td>
									</tr>
						...
						echo "ORganizer is Less then 20 ";
					}
					?>
					</div>
...
<!--pagenation -->
	...
	<a href="organizer.php">&laquo;</a>
  <a href="organizer.php">1</a>
  <a class="active" href="organizer1.php">2</a>
  <a href="organizer2.php">3</a>
  <a href="organizer2.php">&raquo;</a>
</div>
</div>
	<!-- Footer -->
